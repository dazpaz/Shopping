...
using Shopping.CheckoutService.Model;
...
namespace Shopping.CheckoutService
{
	internal sealed class CheckoutService : StatefulService, ICheckoutService
	{
		...
		{
			return ActorProxy.Create<IUserActor>(new ActorId(userId), new Uri("fabric:/Shopping/UserActorService"));
		}

		private IProductCatalogService GetProductCatalogService()
		{
			return ServiceProxy.Create<IProductCatalogService>(
				new Uri("fabric:/Shopping/Shopping.ProductCatalog"), new ServicePartitionKey(0));
		}
